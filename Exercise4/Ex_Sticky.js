var addStickyModule = (function() {
  ...
  function renderData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      domSticky += '<li class="menu-item menu-item-group">' +
        '<h2 class="title-sticky">' +
        data[i].title +
        '</h2>' +
        '<ul class="menu-list menu-list-data">';
      for (var j = 0; j < data[i].subData.length; j++) {
        domSticky += '<li class="menu-item menu-item-data">' + data[i].subData[j] + '</li>';
      }
      domSticky += '</ul>' + '</li>';
    }
  ...
  function addFunctionForSticky() {
    for (var i = 0; i < $sticky.length; i++) {
      ...
      if ($stickyList.offsetHeight >= $sticky[i].offsetTop + $sticky[i].offsetHeight + 45) {
